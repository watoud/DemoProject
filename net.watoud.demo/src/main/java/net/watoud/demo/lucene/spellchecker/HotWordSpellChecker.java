...
import net.watoud.demo.common.Constants;
 ...
 *
 */
public class HotWordSpellChecker implements java.io.Closeable
{

	/**
	 * The default minimum score to use, if not specified by calling
	 * {@link #setAccuracy(float)} .
	 */
	public static final float DEFAULT_ACCURACY = 0.8f;
	 ...
	 */
	public HotWordSpellChecker(Directory spellIndex, StringDistance sd) throws IOException
	{
	 ...
	 *             if spellchecker can not open the directory
	 */
	public HotWordSpellChecker(Directory spellIndex) throws IOException
	{
	 ...
	 */
	public HotWordSpellChecker(Directory spellIndex, StringDistance sd,
	...
	public void setSpellIndex(Directory spellIndexDir) throws IOException
	{
		...
		synchronized (modifyCurrentIndexLock)
		{
			...
			{
				IndexWriter writer = new IndexWriter(spellIndexDir, new IndexWriterConfig(
				        Constants.CURRENT_LUCENE_VERSION, null));
				writer.close();
			}
	 ...
	 *      float)
	 */
	public HotWord[] suggestSimilar(String word, int numSug) throws IOException
	{
	 ...
	 *      float)
	 */
	public HotWord[] suggestSimilar(String word, int numSug, float accuracy) throws IOException
	{
		return this.suggestSimilar(word, numSug, null, null, SuggestMode.SUGGEST_WHEN_NOT_IN_INDEX,
		        accuracy);
	}
	 ...
	 * 
	 */
	public HotWord[] suggestSimilar(String word, int numSug, IndexReader ir, String field,
	        ...
	        SuggestMode suggestMode, float accuracy) throws IOException
	{
		...
		try
		{
			...
			{
				// FIXME
				return new HotWord[] { new HotWord(word, -1) };
			}
			...
			SuggestHotWordQueue sugQueue = new SuggestHotWordQueue(numSug);

			// go thru more than 'maxr' matches in case the distance filter
			// triggers
			int stop = Math.min(hits.length, maxHits);
			SuggestHotWord sugWord = new SuggestHotWord();
			for (int i = 0; i < stop; i++)
			{

				sugWord.word = indexSearcher.doc(hits[i].doc).get(F_WORD);

				String weightStr = indexSearcher.doc(hits[i].doc).get(F_WEIGHT);
				sugWord.weight = Long.valueOf(weightStr);

				// don't suggest a word for itself, that would be silly
				if (sugWord.word.equals(word))
				{
					continue;
				}

				// edit distance
				sugWord.score = sd.getDistance(word, sugWord.word);
				...
				if (ir != null && field != null)
				{ // use the user index
					sugWord.freq = ir.docFreq(new Term(field, sugWord.word)); // freq
					                                                          // in
					                                                          // the
					                                                          // index
				...
				sugWord = new SuggestHotWord();
			}

			// convert to array string
			HotWord[] list = new HotWord[sugQueue.size()];
			for (int i = sugQueue.size() - 1; i >= 0; i--)
			{
				SuggestHotWord suggestHotWord = sugQueue.pop();
				list[i] = new HotWord(suggestHotWord.word, suggestHotWord.weight);
			}
	...
	public void clearIndex() throws IOException
	{
		synchronized (modifyCurrentIndexLock)
		{
			...
			final IndexWriter writer = new IndexWriter(dir, new IndexWriterConfig(
			        Constants.CURRENT_LUCENE_VERSION, null).setOpenMode(OpenMode.CREATE));
			writer.close();
			swapSearcher(dir);
		}
	}
	        ...
	        throws IOException
	{
		synchronized (modifyCurrentIndexLock)
		{
			...
			try
			{
				BytesRefIterator primaryItr = dict.getWordsIterator();
				if (!(primaryItr instanceof TermFreqIterator))
				{
					throw new IllegalArgumentException(
					        "This method does not support non-TermFreqIterator iterator.");
				}

				TermFreqIterator iter = (TermFreqIterator) primaryItr;
				...
				terms: while ((currentTerm = iter.next()) != null)
				{
					...
					// ok index the word and weight
					long weight = iter.weight();
					Document doc = createDocument(word, weight, getMin(len), getMax(len));
	...
	private static Document createDocument(String text, long weight, int ng1, int ng2)
	{
		...
		// TF/pos
		Field f_word = new StringField(F_WORD, text, Field.Store.YES);
		doc.add(f_word); // orig term
		Field f_weight = new LongField(F_WEIGHT, weight, Field.Store.YES);
		doc.add(f_weight);
